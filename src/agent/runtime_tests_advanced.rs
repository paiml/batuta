...
async fn test_max_iterations_with_tool_calls() {
    ...
    // Two tool-call responses that will exhaust iterations
...
async fn test_tool_timeout_produces_error() {
    ...
    // The timed-out tool call still counts in the guard
...
async fn test_retry_exhaustion() {
    ...
    // Driver that always fails with retryable error
    let driver = RetryDriver::new(
        100, // more fails than MAX_RETRIES
        CompletionResponse {
...
async fn test_consecutive_max_tokens_circuit_break() {
    ...
    // 6 consecutive MaxTokens responses — should circuit-break at 5
