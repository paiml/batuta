...
//!
//! These tests validate the OpenAI-compatible chat completion API using
//! raw HTTP requests via reqwest. Validates response structure, content,
//! latency, and SSE streaming.
...
async fn l2_chat_completion_valid_structure() {
    ...
    assert!(!json["choices"].as_array().unwrap().is_empty(), "Should have at least 1 choice");
    assert!(json["choices"][0]["message"]["content"].is_string(), "Choice should have content");
    handle.abort();
}
...
async fn l2_chat_latency_under_budget() {
    ...
    assert_eq!(resp.status(), 200);
    assert!(latency < Duration::from_secs(10), "Latency exceeded 10s: {latency:?}");
    handle.abort();
}
...
async fn l2_chat_streaming_sse() {
    ...
    assert!(
        ct.unwrap_or("").contains("text/event-stream"),
