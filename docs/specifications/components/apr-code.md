...
| **CUDA five-whys: Q4K GEMV produces semi-coherent but wrong output** | (2026-04-05 deep investigation) CPU: `"Okay, the user said \"Say hello.\" I"`. GPU: `",\"\nThe answer, and the number, and the"`. Not total garbage — semi-coherent fragments, systematically wrong from token 1. GQA detected correctly (num_heads=16, num_kv_heads=8). First-layer Q/K/V values match CPU (PAR-058-ATTN debug). FP8 cache is NOT the cause (`--no-fp8-cache` same result). Bug is downstream of QKV projection — likely attention output projection or FFN CUDA matmul. Root cause: the Q4K GEMV kernel itself may have a precision or stride issue specific to Qwen3's hidden_dim=2048 / intermediate_size. **Fix required in realizar CUDA kernels.** | PMAT-181 |
