...
| [apr-code.md](components/apr-code.md) | `apr code` / `batuta code` — Sovereign-only agentic coding assistant. Phases 1-3g DONE: 7 tools, multi-turn history, session persistence (JSONL), model discovery (APR-preferred, Jidoka-validated — PMAT-150), chat templates, auto-compaction, output sanitization, `apr_model_validity` contract, `/test`+`/quality` commands, `--resume`/`--project` flags. All inference local via realizar (GGUF/APR). | `src/agent/`, `src/cli/code.rs`, `src/agent/session.rs` |
