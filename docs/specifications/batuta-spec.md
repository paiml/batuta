...
**Provable contracts:** `apr-code-v1` (sovereignty, tool safety, session integrity, APR.md compliance), `apr-model-discovery-v1` (search order, mtime-first sort, Jidoka validation), `chat-template-v1` (template trait, architecture-aware selection, thinking suppression), `http-api-v1` (OpenAI body schema, max_tokens cap), `session-v1` (JSONL roundtrip, resume fidelity), `tokenizer-v1` (deterministic encode, thread safety). 46+ FALSIFY enforcement tests.
